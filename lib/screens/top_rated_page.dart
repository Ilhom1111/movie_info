...
class TopRatedAllPage extends StatefulWidget {
  const TopRatedAllPage({super.key});

  @override
  State<TopRatedAllPage> createState() => _TopRatedAllPageState();
}

class _TopRatedAllPageState extends State<TopRatedAllPage> {
  int page = 1;
  final _controller = ScrollController();
  PopularModel? _topRated;
  @override
  void initState() {
    context.read<HomeBloc>().add(TopRatedParsEvent(page: 1));
  ...
  void newPage() {
    if (_controller.position.pixels == _controller.position.maxScrollExtent) {
      if (page != 500) {
        if (page != _topRated!.total_pages) {
          page++;
          log(page.toString());
          context.read<HomeBloc>().add(TopRatedParsEvent(page: page));
        }
      }
          ...
          listeners: [
            BlocListener<HomeBloc, HomeState>(
              listener: (context, state) async {
                ...
                if (state is CreateTopRatedSuccess) {
                        ...
                        Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              if (_topRated!.total_pages <= 500)
                                Text(
                                  "Total pages: ${_topRated!.total_pages}",
                                  style: TextStyle(
                                    color: AppColors.xFF92929D,
                                    fontFamily: Fonts.montserrat,
                                    fontSize: 13.sp,
                        ...
                        Expanded(
                          child: GridView.builder(
                            ...
                            itemBuilder: (context, index) {
                              final pop = _topRated!.results[index];
