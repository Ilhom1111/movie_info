...
class UpComingAllPage extends StatefulWidget {
  const UpComingAllPage({super.key});

  @override
  State<UpComingAllPage> createState() => _UpComingAllPageState();
}

class _UpComingAllPageState extends State<UpComingAllPage> {
  int page = 1;
  final _controller = ScrollController();
  NowPlaying? _upComing;
  @override
  void initState() {
    context.read<HomeBloc>().add(UpComingParsEvent(page: 1));
  ...
  void newPage() {
    if (_controller.position.pixels == _controller.position.maxScrollExtent) {
      if (page != 500) {
        if (page != _upComing!.total_pages) {
          page++;
          log(page.toString());
          context.read<HomeBloc>().add(UpComingParsEvent(page: page));
        }
      }
          ...
          listeners: [
            BlocListener<HomeBloc, HomeState>(
              listener: (context, state) async {
                ...
                if (state is CreateUpComingSuccess) {
                        ...
                        Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              if (_upComing!.total_pages <= 500)
                                Text(
                                  "Total pages: ${_upComing!.total_pages}",
                                  style: TextStyle(
                                    color: AppColors.xFF92929D,
                                    fontFamily: Fonts.montserrat,
                                    fontSize: 13.sp,
                        ...
                        Expanded(
                          child: GridView.builder(
                            ...
                            itemBuilder: (context, index) {
                              final pop = _upComing!.results[index];
